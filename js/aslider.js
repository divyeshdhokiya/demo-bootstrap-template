    ...
    $.fn.aslider = function (options) {
        ...
        this.previewSlide = function (slide) {
            ...
            if (slide == 'prev') {
                _.find('div.slide:last-child').find('img').clone().
                    insertAfter('.aslider-' + slide).addClass('slide-preview preview-' + slide).animate({right: '92.8%'}, 200);
            } else if (slide == 'next') {
                _.find('div.active').next('div.slide').find('img').clone().
                    insertAfter('.aslider-' + slide).addClass('slide-preview preview-' + slide).animate({left: '92.8%'}, 200);
            }

            _.find('.aslider-' + slide).on('mouseleave', function () {
                if (slide == 'prev') {
                    $('.slide-preview').animate({right: '100%'}, 200, function () {
                        this.remove();
                    });
                } else if (slide == 'next') {
                    $('.slide-preview').animate({left: '100%'}, 200, function () {
                        this.remove();
                    });
                }
            });
        ...
        _.find('.aslider-nav').on('click', function () {
            ...
            if (settings.preview == true) {
                ...
                $('.slide-preview').fadeOut(200, function () {
                    this.remove();
                });
